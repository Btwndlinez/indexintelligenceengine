...
export async function POST(_req: NextRequest) {
  try {
    const [companiesRes, bidsRes, equipmentRes] = await Promise.all([
      db.supabaseFetch("/rest/v1/deep_profiles?select=company_name,city,updated_at&order=updated_at.desc&limit=5"),
      db.supabaseFetch("/rest/v1/bid_results?select=title,agency,city,created_at&order=created_at.desc&limit=3"),
      db.supabaseFetch("/rest/v1/deep_profiles?select=company_name,city,updated_at&order=updated_at.desc&limit=5"),
    ]);
