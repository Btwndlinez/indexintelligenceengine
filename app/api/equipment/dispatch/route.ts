...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      console.info('Equipment dispatch (email fallback)', { to: dispatch_email, subject, organizationId: tenant.organizationId });
      return NextResponse.json({ success: true, method: 'logged' });
    }

    return NextResponse.json({ success: true, method: 'email' });
  } catch (err: any) {
    console.warn('Equipment dispatch failed, using fallback', err.message);
    return NextResponse.json({ success: true, method: 'logged' });
  }
}
